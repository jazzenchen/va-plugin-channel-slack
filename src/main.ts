...
function log(level: string, msg: string): void {
  process.stderr.write(`[slack-plugin][${level}] ${msg}\n`);
}
...
async function start(): Promise<void> {
  ...
  const { agent, meta, agentInfo, conn } = await connectToHost(
    { name: "vibearound-slack", version: "0.1.0" },
    (_a) => ({
  ...
  if (!botToken) {
    throw new Error("bot_token is required in Slack config");
  }
  if (!appToken) {
    throw new Error("app_token is required in Slack config (Socket Mode)");
  }

  log("info", `initialized, host=${agentInfo.name ?? "unknown"} cacheDir=${cacheDir}`);

  // Create Slack bot
  const slackBot = new SlackBot({ bot_token: botToken, app_token: appToken }, agent, log, cacheDir);
  ...
  streamHandler = new AgentStreamHandler(slackBot, log, {
    showThinking: verbose?.show_thinking ?? false,
    showToolUse: verbose?.show_tool_use ?? false,
  });
  slackBot.setStreamHandler(streamHandler);
